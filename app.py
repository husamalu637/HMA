...
async function checkVideoLink() {
    ...
    statusDiv.innerHTML = "⏳ جاري معالجة الرابط واستخراج الجودات...";
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            statusDiv.innerHTML = "✨ تم العثور على جودات الفيديو!";
        } else {
            statusDiv.innerHTML = `❌ خطأ: ${result.error || 'فشل جلب الجودات'}`;
...
async function startDownload() {
    ...
    statusDiv.innerHTML = "📥 جاري تحويلك للتحميل المباشر...";
    
    // الحل: فتح الرابط مباشرة لضمان عدم حدوث إخفاق
    window.open(selectedMedia.url, '_blank');
    
    statusDiv.innerHTML = "✅ جاري التنزيل من مصدر الفيديو الأصلي...";
}
</script>
...
def check_api():
    ...
    try:
        ...
        medias = result['medias'] if result and 'medias' in result else [{"url": result.get('url', ''), "quality": "تلقائية", "extension": "mp4"}]
